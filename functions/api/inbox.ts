...
import {
  readInboxUpdateInput,
  updateInbox,
  validateInboxUpdateRequest,
} from "../_shared/inboxUpdate.ts";
...
export const onRequest = async (context: FunctionContext): Promise<Response> => {
  if (context.request.method === "PATCH") {
    const guard = validateInboxUpdateRequest(context.request);
    if (guard) return Response.json({ error: guard.error }, { status: guard.status, headers });
    const input = await readInboxUpdateInput(context.request);
    if (!input.ok) return Response.json({ error: input.error }, { status: input.status, headers });
    try {
      return Response.json(await updateInbox(context.env, input.value), { headers });
    } catch (error) {
      const failure = publicError(error);
      console.error(`inbox update failed: ${failure.code}`);
