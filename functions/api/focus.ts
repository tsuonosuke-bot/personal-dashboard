...
export const onRequest = async (context: FunctionContext): Promise<Response> => {
  ...
  if (action !== "focus-update" && action !== "focus-reorder" && action !== "focus-swap") {
  ...
  try {
    ...
    if (action === "focus-swap") {
      const input = await readFocusSwapInput(context.request);
      if (!input.ok) return Response.json({ error: input.error }, { status: input.status, headers });
      return Response.json({ items: await swapFocusItems(context.env, input.value) }, { headers });
    }
